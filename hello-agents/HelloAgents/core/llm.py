...
class HelloAgentsLLM:
    ...
    def think(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        try:
            params = self._build_create_kwargs(
                messages=messages,
            ...
            response = self._create_with_token_compat(params)

            # 处理流式响应
            print("✅ 大语言模型响应成功:")
            for chunk in response:
                # 某些 provider（含 OpenAI 启用 usage 时）会返回 choices=[] 的尾包
    ...
    def _build_create_kwargs(self, messages, temperature, max_tokens, extra: dict, stream: bool) -> dict:
        """组装 chat.completions.create 的入参；max_tokens 仅在显式给出时透传"""
        params = {
            "model": self.model,
            "messages": messages,
    ...
    def _create_with_token_compat(self, params: dict):
        """兼容新老模型：max_tokens 不被支持时改用 max_completion_tokens 重试一次"""
        try:
