...
class HelloAgentsLLM:
    ...
    设计理念：
    - 统一配置：优先使用 LLM_MODEL_ID、LLM_API_KEY、LLM_BASE_URL、LLM_TIMEOUT
    - 兼容旧配置：同时识别 MODEL_ID、API_KEY、BASE_URL
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            model: 模型名称，默认从 LLM_MODEL_ID/MODEL_ID 读取
            api_key: API密钥，默认从 LLM_API_KEY/API_KEY 读取
            base_url: 服务地址，默认从 LLM_BASE_URL/BASE_URL 读取
            app_id: 旧版 API 密钥参数名，等同于 api_key
            provider: 服务提供方名称；当前保留用于兼容旧调用
        ...
        self.api_key = api_key or app_id or os.getenv("LLM_API_KEY") or os.getenv("API_KEY")
        self.base_url = base_url or os.getenv("LLM_BASE_URL") or os.getenv("BASE_URL")
