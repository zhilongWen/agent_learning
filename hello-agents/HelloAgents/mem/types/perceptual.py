...
from mem.base import BaseMemory, MemoryConfig, MemoryItem
...
class PerceptualMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # 可选加载：图像CLIP与音频CLAP。默认不加载，避免初始化时联网下载模型。
        ...
        self._init_optional_multimodal_models()

        # 向量存储（Qdrant）— 按模态拆分集合，避免维度冲突，使用连接管理器避免重复连接
        from mem.storage import QdrantConnectionManager
    ...
    def _init_optional_multimodal_models(self):
        """按需初始化多模态模型；默认使用哈希编码，避免隐式联网。"""
        if os.getenv("PERCEPTUAL_ENABLE_CLIP", "0").lower() in {"1", "true", "yes"}:
            self._load_clip_model()
        if os.getenv("PERCEPTUAL_ENABLE_CLAP", "0").lower() in {"1", "true", "yes"}:
            self._load_clap_model()

    def _model_local_only(self) -> bool:
        return os.getenv("PERCEPTUAL_MODEL_LOCAL_ONLY", "1").lower() not in {"0", "false", "no"}

    def _load_clip_model(self):
        try:
            from transformers import CLIPModel, CLIPProcessor
            clip_name = os.getenv("CLIP_MODEL", "openai/clip-vit-base-patch32")
            local_only = self._model_local_only()
            self._clip_model = CLIPModel.from_pretrained(clip_name, local_files_only=local_only)
            self._clip_processor = CLIPProcessor.from_pretrained(clip_name, local_files_only=local_only)
            self._image_dim = getattr(self._clip_model.config, "projection_dim", None) or 512
        except Exception as e:
            self._clip_model = None
            self._clip_processor = None
            self._image_dim = self.vector_dim
            logger.info(f"CLIP未启用或不可用，图像感知记忆使用哈希编码: {e}")
    ...
    def _load_clap_model(self):
        try:
            ...
            local_only = self._model_local_only()
            self._clap_model = ClapModel.from_pretrained(clap_name, local_files_only=local_only)
            self._clap_processor = ClapProcessor.from_pretrained(clap_name, local_files_only=local_only)
            self._audio_dim = getattr(self._clap_model.config, "projection_dim", None) or 512
        except Exception as e:
            self._clap_model = None
            self._clap_processor = None
            self._audio_dim = self.vector_dim
            logger.info(f"CLAP未启用或不可用，音频感知记忆使用哈希编码: {e}")
