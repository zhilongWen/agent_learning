...
class ReflectionAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            custom_prompts: 旧版提示词模板字典，支持 initial/reflect/refine
        """
        # 默认 system_prompt
...
请始终保持批判性思维，追求更高质量的输出。"""
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        try:
            ...
            print(f"❌ ReflectionAgent 执行失败: {e}")
            final_result = self.memory.get_last_execution() or f"❌ ReflectionAgent 执行失败: {e}"
    ...
    def _execute_task(self, task: str, **kwargs) -> str:
        """执行初始任务"""
        prompt = self.custom_prompts.get("initial")
        user_content = (
            self._render_custom_prompt(prompt, task=task)
            if prompt
            else f"请完成以下任务：\n\n{task}"
        )
        messages = [
    ...
    def _reflect_on_result(self, task: str, result: str, **kwargs) -> str:
        """对结果进行反思"""
        prompt = self.custom_prompts.get("reflect")
        user_content = (
            self._render_custom_prompt(prompt, task=task, content=result)
            if prompt
            else f"""请仔细审查以下回答，并找出可能的问题或改进空间：
...
如果回答已经很好，请回答"无需改进"。"""
    ...
    def _refine_result(self, task: str, last_attempt: str, feedback: str, **kwargs) -> str:
        """根据反馈优化结果"""
        prompt = self.custom_prompts.get("refine")
        user_content = (
...
请提供一个改进后的回答。"""
    ...
    def _render_custom_prompt(self, template: str, **values: str) -> str:
        """Render legacy custom prompt templates with a safe fallback."""
        try:
