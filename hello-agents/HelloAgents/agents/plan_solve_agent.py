...
请专注于解决当前步骤，并输出该步骤的最终答案。"""
    ...
    def execute(self, question: str, plan: List[str], **kwargs) -> str:
        ...
        print("\n--- 正在执行计划 ---")
        executable_plan = plan[:self.max_steps] if self.max_steps is not None else plan
        if self.max_steps is not None and len(plan) > self.max_steps:
            print(f"⚠️ 计划共 {len(plan)} 步，本次最多执行前 {self.max_steps} 步。")

        for i, step in enumerate(executable_plan, 1):
            print(f"\n-> 正在执行步骤 {i}/{len(executable_plan)}: {step}")
...
class PlanSolveAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            max_steps: 最多执行的计划步骤数（默认执行完整计划）
            custom_prompts: 旧版提示词字典，支持 planner/executor
        """
        if custom_prompts:
            planner_prompt = planner_prompt or custom_prompts.get("planner")
            executor_prompt = executor_prompt or custom_prompts.get("executor")
